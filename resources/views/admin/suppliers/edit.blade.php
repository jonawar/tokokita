                ...
                <form action="{{ route('admin.suppliers.update', $supplier->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-1">
                            ...
                            <input type="text" name="name" value="{{ $supplier->name }}" required class="w-full p-3 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                        </div>
                        <div class="md:col-span-1">
                            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-widest">Kontak / Tlp</label>
                            <input type="text" name="phone" value="{{ $supplier->phone }}" class="w-full p-3 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-widest">Email</label>
                            <input type="email" name="email" value="{{ $supplier->email }}" class="w-full p-3 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-widest">Alamat</label>
                            <textarea name="address" rows="4" class="w-full p-4 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium italic">{{ $supplier->address }}</textarea>
                        </div>
                    </div>
